...
bool spiFlashInit(void)
{
  ...
  if (ret)
  {
    ...
    {
      logPrintf("[E_] spiFlashGetID()\n");
      ret = false;
    }
  }
  else
  {
    logPrintf("[E_] spiFlashReset()\n");
  }
  is_init = ret;
...
bool spiFlashWrite(uint32_t addr, uint8_t *p_data, uint32_t length)
{
  ...
  current_size = W25Q32JV_PAGE_SIZE - (addr % W25Q32JV_PAGE_SIZE);
  ...
  do
  {
    ...
    p_data += current_size;
    current_size = ((current_addr + W25Q32JV_PAGE_SIZE) > end_addr) ? (end_addr - current_addr) : W25Q32JV_PAGE_SIZE;
...
bool spiFlashErase(uint32_t addr, uint32_t length)
{
  ...
  flash_length = W25Q32JV_FLASH_SIZE;
  block_size   = W25Q32JV_SECTOR_SIZE;
...
bool spiFlashEraseBlock(uint32_t block_addr)
{
  ...
  ret &= spiFlashWaitBusy(W25Q32JV_SECTOR_ERASE_MAX_TIME);
...
bool spiFlashEraseSector(uint32_t sector_addr)
{
  ...
  ret &= spiFlashWaitBusy(W25Q32JV_SUBSECTOR_ERASE_MAX_TIME);
...
bool spiFlashWaitBusy(uint32_t timeout)
{
  ...
  while(1)
  {
    ...
    if (ret == true)
    {
      ...
      busy_bit = rx_buf[1] & W25Q32JV_SR_WIP;
      if (busy_bit == 0)
      {
        break;
      }
